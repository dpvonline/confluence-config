...
class BackupManager(object):
    def __init__(self, params):
        ...
        self._db_user = params['POSTGRES_USER']
        self._db_password = params['POSTGRES_PASSWORD']
        self._database = params['POSTGRES_DB']
        self._db_server = params['POSTGRES_HOST']

    def __activate_maintenance(self):
        DockerUtils.run_cmd(cmd="docker stop confluence_app", container=DockerContainer.BACKUP)

    def __deactivate_maintenance(self):
        DockerUtils.run_cmd(cmd="docker start confluence_app",
    ...
    def __backup_database(self, backup_dir: str):
        ...
        cmd = "PGPASSWORD={pwd} pg_dump -h {server} -Fc -U {user} {db}> {path}/backup.sql".format(
    ...
    def __restore_database(self, backup_dir: str):
        # Restore
        cmd_restore = "PGPASSWORD={pwd} pg_restore -c -h {server} -U {user} -d {db} {path}/backup.sql".format(
            server=self._db_server,
            user=self._db_user,
            pwd=self._db_password,
            db=self._database,
